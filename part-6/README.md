...
To get the right path RESTCONF path or data resource, we can check from the webgui under the config editor. Change the output to RESTCONF. Below are the sample
...
```
To see thru what other data resource available, below are the script. The depth can be adjusted

```
import requests

url = "http://IP-ADDRESS:8080/restconf/data?depth=1"
...
At this point we been using config data resource to make configuration and validate. Next, we can see the operations state using different data resource.
